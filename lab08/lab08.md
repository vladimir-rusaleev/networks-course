...
За 1 RTT окно перегрузки увеличивается на 1 MSS, значит всего для увеличения с W/2 до W потребуется T = W/2 RTT.
Средний размер окна на этом интервале тогда будет равен 3/4 * W RTT. Средняя пропускная способность throughput = $\frac{W_{avg} * MSS}{RTT} = \frac{3 * W * MSS}{4 * RTT}$. Выразим W: W = 2 * T, подставим сюда: throughput = $\frac{3 * T * MSS}{2 * RTT}$. Выразим T:
T = $\frac{throughput * 2 * RTT}{3 * MSS}$
