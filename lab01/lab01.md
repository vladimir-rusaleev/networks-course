...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Доп.информация: Host, connection, user-agent - информация о браузере, accept - принимаемые типы файлов, accept-encoding - поддерживаемые кодировки
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - да
   - OK 200, и видим содержимое в line-based text data
   ![alt text](task_2.png)
...
3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - 4 сегмента:
   [4 Reassembled TCP Segments (4864 bytes): #101(1448), #102(1448), #103(1448), #104(520)]
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - нет такого, только общая длина Content-Length: 4500

   ![alt text](task_3.png)

   ![alt text](task_3_1.png)
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 GET-запроса
   - два на 128.119.245.12, один на 2.56.99.24
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Параллельно, т.к сначала были сделаны запросы на обе картинки, а затем только эти картинки были получены(а не сначала сделали запрос на первую картинку, дождались ее получения, и только потом сделали запрос на вторую)

   ![alt text](task_4_1.png)
